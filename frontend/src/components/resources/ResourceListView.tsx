...
type Props = {
  ...
  /** Ruta donde vive la tabla: los hrefs de filtros/orden/paginación se
   * construyen sobre ella (la página genérica o una anfitriona como /backups). */
  basePath: string;
  rawSearchParams: Record<string, string | string[] | undefined>;
};

/**
 * Ensamblaje completo de la tabla genérica de un recurso (toolbar + filtros +
 * tabla + totales + paginación), reutilizable fuera de /resources/[name]:
 * cualquier página anfitriona puede incrustar el listado de un recurso del
 * contrato pasando su capability y su propia ruta base.
 */
export async function ResourceListView({ capability, basePath, rawSearchParams }: Props) {
  if (capability.view !== "table" || !capability.list) {
    return null;
  }
  const resourceName = capability.name;
